...
/** \fn long timespec_to_micros(struct timespec ts)
...
/** \fn long timespec_to_micros_float(struct timespec ts)
 *  \brief Converts a timespec to a floating point number of microseconds.
*/
float timespec_to_micros_float(struct timespec ts)
{
  return ((float)ts.tv_sec * 1e6) + ((float)ts.tv_nsec / 1000);
}
